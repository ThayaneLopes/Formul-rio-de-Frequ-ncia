...
<body>
	...
	<div class="menu">
		...
		<table border=1; style="width: 100%"
			id="tabelaListaFormularioBolsista">
			<tr>
				<td>Mês e Ano</td>
				...
				<tr>
					...
					<c:choose>
						<c:when test="${formularioFrequencia.statusBolsista == false}">
							<td><a
								href="EnviarAoOrientador?id=${formularioFrequencia.id}"
								class="btn btn-info">Enviar ao Orientador</a><a
								href="exibirAlterarFormularioBolsista?id=${formularioFrequencia.id}"
						...
						<c:when test="${formularioFrequencia.statusBolsista == true}">
							<td><a class="btn btn-info" disabled>Enviado ao
									Orientador</a><a class="btn btn-warning" disabled>Alterar</a></td>
						</c:when>
					</c:choose>
